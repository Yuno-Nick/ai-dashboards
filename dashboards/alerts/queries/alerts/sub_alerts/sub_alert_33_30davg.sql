-- ==============================================================================
-- Alert 3.3: Quality Rate Drop - vs 30d Same Weekday Average (SAME MOMENT)
-- ==============================================================================
...
baseline_30d_same_weekday AS (
    SELECT
        ...
        ROUND(AVG(good_calls), 0) AS avg_good_calls,
    ...
    FROM (
        SELECT
            ...
            SUM(CASE WHEN d.call_classification IN ('good_calls', 'short_calls', 'completed') 
                     THEN 1 ELSE 0 END) AS completed_calls,
            SUM(CASE WHEN d.call_classification = 'good_calls' THEN 1 ELSE 0 END) AS good_calls,
            ROUND(
...
alert_calculation AS (
    SELECT
        ...
        b.avg_good_calls AS baseline_avg_good_calls,
        b.avg_quality_rate AS baseline_mean,
        b.stddev_quality_rate AS baseline_stddev,
        b.sample_size AS baseline_sample_size,
        
        -- Diferencia en puntos porcentuales
        CASE 
            WHEN b.avg_quality_rate IS NULL THEN NULL
            ELSE ROUND((t.quality_rate - b.avg_quality_rate) * 100, 1)
        END AS pp_change,
        
        -- Z-Score usando stddev del MISMO DÍA DE SEMANA
        CASE 
            WHEN b.stddev_quality_rate IS NULL OR b.stddev_quality_rate = 0 THEN NULL
            ELSE ROUND((t.quality_rate - b.avg_quality_rate) / b.stddev_quality_rate, 2)
        END AS z_score
...
severity_calculation AS (
    SELECT
        *,
        ...
        CASE
            -- Datos insuficientes
            WHEN current_completed_calls < 30 
    ...
    CASE
        WHEN alert_severity = 'CRITICAL' THEN
            CONCAT(
                ...
                CAST(baseline_avg_good_calls AS VARCHAR), '/', CAST(baseline_avg_completed_calls AS VARCHAR), ' good/completed). ',
