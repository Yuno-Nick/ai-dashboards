-- ==============================================================================
-- Alert 3.2: Quality Rate Drop - vs Last Week Same Day (Same Moment)
-- ==============================================================================
...
-- Stddev de quality_rate del MISMO DÍA DE SEMANA (últimos 30d) - para comparación WoW
...
alert_calculation AS (
    SELECT
        ...
        lw.good_calls AS baseline_good_calls,
        lw.quality_rate AS baseline_quality_rate,
        
        -- Stddev del mismo día de semana
        s.stddev_quality_rate AS stddev_same_weekday,
        ...
        CASE 
            ...
            ELSE ROUND((t.quality_rate - lw.quality_rate) / s.stddev_quality_rate, 2)
        END AS z_score
    ...
    CASE
        WHEN alert_severity = 'CRITICAL' THEN
            CONCAT(
                ...
                day_name, ' pasado a esta hora: ', CAST(ROUND(baseline_quality_rate * 100, 1) AS VARCHAR), '% (',
