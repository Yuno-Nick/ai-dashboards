-- ==============================================================================
-- Alert 4: Short Call Rate Spike - ALERT VIEW (Solo alertas CRITICAL y WARNING)
-- ==============================================================================
-- Detecta anomalías en el ratio de llamadas cortas usando estadísticas
-- basadas en los últimos 30 días. Alerta cuando short_call_rate > media + 2σ
-- Usa ai_calls_detail + alerts_baseline_stats
--
-- Esta es la vista de alertas, solo muestra CRITICAL y WARNING con mensaje.
-- Para la vista completa sin filtros, ver: normal_alert_4.sql
-- ==============================================================================
  ...
  SELECT
    ...
    CASE
      ...
      WHEN alert_severity = 'CRITICAL'
        THEN CONCAT('CRITICAL: ', organization_name, ' (', country, ') - Short call rate SPIKE detected! ',
             'Current: ', CAST(ROUND(current_short_call_rate * 100, 1) AS VARCHAR), '% (', CAST(current_short_calls AS VARCHAR), ' short calls) ',
             'vs Baseline: ', CAST(ROUND(baseline_short_call_rate * 100, 1) AS VARCHAR), '% ',
             '(+', CAST(sigma_deviation AS VARCHAR), 'σ deviation)')
      
      WHEN alert_severity = 'WARNING'
        THEN CONCAT('WARNING: ', organization_name, ' (', country, ') - Elevated short call rate. ',
             'Current: ', CAST(ROUND(current_short_call_rate * 100, 1) AS VARCHAR), '% ',
             'vs Baseline: ', CAST(ROUND(baseline_short_call_rate * 100, 1) AS VARCHAR), '% ',
             '(+', CAST(sigma_deviation AS VARCHAR), 'σ)')
      
      ELSE 'FINE: Short call rate within normal range'
